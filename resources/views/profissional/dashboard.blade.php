...
<header class="mb-stack-lg">
    <h2 class="font-headline-lg text-headline-lg text-on-surface">Olá, {{ explode(' ', Auth::user()->name)[0] }}. Pronto para mais um dia?</h2>
...
<div class="grid grid-cols-12 gap-stack-lg">
    ...
    <section class="col-span-12 lg:col-span-8 grid grid-cols-2 md:grid-cols-3 gap-stack-md">
        <div class="col-span-2 md:col-span-3 bg-primary-container text-on-primary-container p-stack-lg rounded-xl flex flex-col md:flex-row md:items-center justify-between shadow-sm relative overflow-hidden group">
            <div class="z-10">
                ...
                <h3 class="font-headline-md text-headline-md font-bold mt-1">
                    {{ $totalConsultas > 0 ? 'Você realizou '.$totalRealizadas.' de '.$totalConsultas.' consultas' : 'A sua agenda está vazia este mês' }}
                </h3>
            ...
            <div class="z-10 mt-4 md:mt-0 flex gap-2">
                <a href="{{ route('profissional.agenda') }}" class="bg-surface-container-lowest text-primary px-4 py-2 rounded-lg font-bold text-label-md hover:scale-105 transition-transform shadow-sm">Ver Agenda</a>
            </div>
        ...
        <div class="bg-white p-stack-md rounded-xl shadow-[0px_4px_12px_rgba(0,0,0,0.05)] flex flex-col justify-between border border-outline-variant/30">
            <div class="flex justify-between items-start">
                ...
                <span class="text-primary font-bold text-label-md flex items-center gap-1">{{ $totalPacientes }} total</span>
            </div>
            <div class="mt-4">
                <p class="font-body-sm text-body-sm text-on-surface-variant">Pacientes</p>
                <p class="font-headline-md text-headline-md font-bold text-on-surface">{{ $totalPacientes }}</p>
            </div>
        </div>
        ...
        <div class="bg-white p-stack-md rounded-xl shadow-[0px_4px_12px_rgba(0,0,0,0.05)] flex flex-col justify-between border border-outline-variant/30">
            <div class="flex justify-between items-start">
                ...
                <span class="text-primary font-bold text-label-md flex items-center gap-1">{{ $consultasHoje }} hoje</span>
            </div>
            <div class="mt-4">
                <p class="font-body-sm text-body-sm text-on-surface-variant">Consultas Hoje</p>
                <p class="font-headline-md text-headline-md font-bold text-on-surface">{{ $consultasHoje }}</p>
            </div>
        </div>
        ...
        <div class="bg-white p-stack-md rounded-xl shadow-[0px_4px_12px_rgba(0,0,0,0.05)] flex flex-col justify-between border border-outline-variant/30">
            <div class="flex justify-between items-start">
                ...
                <span class="text-primary font-bold text-label-md">{{ $proximasConsultas->count() }} agendadas</span>
            </div>
            <div class="mt-4">
                <p class="font-body-sm text-body-sm text-on-surface-variant">Próximas</p>
                <p class="font-headline-md text-headline-md font-bold text-on-surface">{{ $proximasConsultas->count() }}</p>
            </div>
        </div>
    </section>
        ...
        <div class="relative z-10 w-full">
            ...
            <div class="relative w-32 h-32 mb-6 mx-auto">
                <svg class="w-full h-full" viewbox="0 0 36 36">
                    ...
                    <path class="text-white" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="currentColor" stroke-dasharray="{{ $pctRealizadas }}, 100" stroke-linecap="round" stroke-width="3"></path>
                    <text class="fill-white font-bold text-[8px]" text-anchor="middle" x="18" y="20.35">{{ $pctRealizadas }}%</text>
                </svg>
            </div>
            <h3 class="font-title-lg text-title-lg font-bold mb-2">{{ $pctRealizadas >= 70 ? 'Excelente desempenho!' : 'Espaço para melhorar' }}</h3>
            <p class="font-body-sm text-body-sm opacity-90 mb-6">{{ $totalRealizadas }} de {{ $totalConsultas }} consultas realizadas este mês.</p>
            <a href="{{ route('profissional.agenda') }}" class="block w-full bg-white text-secondary py-3 rounded-xl font-bold hover:bg-secondary-fixed transition-colors active:scale-95 shadow-sm text-center">Gerir Agenda</a>
        </div>
    ...
    <section class="col-span-12 lg:col-span-8 bg-white rounded-xl shadow-[0px_4px_12px_rgba(0,0,0,0.05)] border border-outline-variant/30">
        <div class="p-stack-md border-b border-outline-variant/30 flex justify-between items-center">
            ...
            <a class="text-primary font-bold text-label-md hover:underline" href="{{ route('profissional.agenda') }}">Ver agenda</a>
        </div>
                ...
                <div class="flex items-center justify-between px-5 py-4 hover:bg-surface-container-low transition-colors gap-3">
                    <div class="flex items-center gap-4 flex-1 min-w-0">
                        ...
                        <div class="min-w-0">
                            ...
                            <p class="text-body-sm text-on-surface-variant">{{ \Carbon\Carbon::parse($consulta->data)->format('d M Y') }} · {{ \Carbon\Carbon::parse($consulta->hora)->format('H:i') }} · {{ $consulta->tipo }}</p>
                        </div>
                    </div>
                    <div class="flex items-center gap-2 flex-shrink-0">
                        <span class="font-label-md font-semibold px-3 py-1 rounded-full
                            {{ $consulta->estado == 'Agendada' ? 'bg-secondary-container text-on-secondary-container' : '' }}
                            {{ $consulta->estado == 'Realizada' ? 'bg-primary-fixed text-on-primary-fixed' : '' }}
                            {{ $consulta->estado == 'Cancelada' ? 'bg-error-container text-on-error-container' : '' }}
                            {{ $consulta->estado == 'Faltou' ? 'bg-amber-50 text-amber-800' : '' }}">
                            {{ $consulta->estado }}
                        </span>
                        ...
                        @endif
                        @if($consulta->estado === 'Agendada' && $consulta->confirmada)
                            <form method="POST" action="{{ route('profissional.consultas.estado', $consulta) }}" class="inline">
                                @csrf @method('PUT')
        ...
        <div class="p-stack-md text-center text-on-surface-variant text-body-sm py-8">
            ...
            <button onclick="window.location.href='{{ route('profissional.agenda') }}'" class="inline-block bg-primary text-on-primary px-4 py-2 rounded-lg text-label-md font-bold hover:opacity-90 transition-all">Agendar</button>
        </div>
        @endif
    </section>
    ...
    <section class="col-span-12 lg:col-span-4 bg-white rounded-xl shadow-[0px_4px_12px_rgba(0,0,0,0.05)] border border-outline-variant/30">
        <div class="p-stack-md border-b border-outline-variant/30">
            ...
            <p class="text-body-sm text-on-surface-variant mt-0.5">{{ now()->translatedFormat('F Y') }}</p>
        </div>
        <div class="p-stack-md space-y-3">
            @if($consultasMes->isNotEmpty())
                @foreach(['Agendada' => 'secondary-container', 'Realizada' => 'primary-fixed', 'Faltou' => 'amber-100', 'Cancelada' => 'error-container'] as $estado => $cor)
                    @if(isset($consultasMes[$estado]))
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-2">
                            <span class="w-2.5 h-2.5 rounded-full bg-{{ str_replace('-', '-', explode('-', $cor)[0]) }}-400"></span>
                            <span class="text-body-sm text-on-surface-variant">{{ $estado }}</span>
                        </div>
                        <span class="text-body-sm font-bold text-on-surface">{{ $consultasMes[$estado] }}</span>
                    </div>
                    @endif
                @endforeach
                ...
                <div class="flex items-center justify-between">
                    ...
                    <span class="text-body-sm font-bold text-primary">{{ $consultasMes->sum() }}</span>
                </div>
            @else
    ...
    <section class="col-span-12 lg:col-span-4 bg-white rounded-xl shadow-[0px_4px_12px_rgba(0,0,0,0.05)] border border-outline-variant/30">
        ...
        <div class="p-stack-md space-y-3">
            <a href="{{ route('profissional.mensagens.index') }}" class="flex items-center gap-4 p-3 rounded-xl hover:bg-surface-container-low transition-colors group">
                ...
                <div>
                    ...
                    <p class="text-body-sm text-on-surface-variant">{{ $mensagensNaoLidas }} não lida(s)</p>
                </div>
            </a>
            <a href="{{ route('profissional.pacientes.index') }}" class="flex items-center gap-4 p-3 rounded-xl hover:bg-surface-container-low transition-colors group">
                ...
                <div>
                    ...
                    <p class="text-body-sm text-on-surface-variant">{{ $totalPacientes }} no total</p>
                </div>
            </a>
            <a href="{{ route('profissional.documentos.index') }}" class="flex items-center gap-4 p-3 rounded-xl hover:bg-surface-container-low transition-colors group">
    ...
    <section class="col-span-12 lg:col-span-8 bg-white rounded-xl shadow-[0px_4px_12px_rgba(0,0,0,0.05)] border border-outline-variant/30">
        <div class="p-stack-md border-b border-outline-variant/30 flex justify-between items-center">
            ...
            <a href="{{ route('profissional.agenda') }}" class="text-primary font-bold text-label-md hover:underline">Ver tudo</a>
        </div>
        <div class="p-stack-md text-center text-on-surface-variant text-body-sm py-6">
            ...
            <a href="{{ route('profissional.agenda') }}" class="inline-block mt-4 bg-primary text-on-primary px-4 py-2 rounded-lg text-label-md font-bold hover:opacity-90 transition-all">Ir para Agenda</a>
        </div>
    </section>
</div>
@endsection
