...
@section('title', $paciente->user?->name ?? 'Paciente')
...
<div class="max-w-5xl mx-auto">
        ...
        <div class="flex items-center gap-5">
            @if($paciente->user?->foto_perfil)
                <img src="{{ asset('storage/' . $paciente->user->foto_perfil) }}" alt="Foto"
                     class="w-16 h-16 rounded-full object-cover" loading="lazy" decoding="async">
            @else
            ...
            <div class="flex-1">
                ...
                <p class="text-body-sm text-on-surface-variant">{{ $paciente->user?->email }}</p>
                <div class="flex items-center gap-3 mt-2">
                    @if($paciente->subscricaoActiva?->plano)
                        <span class="font-label-md font-semibold px-2.5 py-1 rounded-full bg-primary-fixed text-on-primary-fixed text-[11px]">
                            {{ $paciente->subscricaoActiva->plano->nome }}
                        </span>
                    @endif
                    <span class="text-xs text-on-surface-variant flex items-center gap-1">
                        <span class="material-symbols-outlined text-[14px]">calendar_today</span>
                        Desde {{ $paciente->data_inicio?->format('d/m/Y') ?? '—' }}
                    </span>
                </div>
            </div>
            <a href="{{ route('profissional.pacientes.ficha', $paciente) }}"
    ...
    <div class="portal-tabs mb-6">
        <a href="{{ route('profissional.pacientes.show', ['paciente' => $paciente, 'tab' => 'info']) }}"
           class="portal-tab {{ $tab === 'info' ? 'active' : '' }} flex items-center gap-1">
            <span class="material-symbols-outlined text-[18px]">info</span> Informações
        </a>
        <a href="{{ route('profissional.pacientes.show', ['paciente' => $paciente, 'tab' => 'consultas']) }}"
           class="portal-tab {{ $tab === 'consultas' ? 'active' : '' }} flex items-center gap-1">
            <span class="material-symbols-outlined text-[18px]">calendar_month</span> Consultas
        </a>
        <a href="{{ route('profissional.pacientes.show', ['paciente' => $paciente, 'tab' => 'plano']) }}"
           class="portal-tab {{ $tab === 'plano' ? 'active' : '' }} flex items-center gap-1">
            ...
            <div class="bg-white rounded-xl shadow-[0px_4px_12px_rgba(0,0,0,0.05)] border border-outline-variant/30 p-6">
                ...
                <dl class="space-y-3 text-body-sm">
                    <div class="flex justify-between">
                        ...
                        <dd class="text-on-surface font-medium">{{ $paciente->user?->name ?? '—' }}</dd>
                    </div>
                    <div class="flex justify-between">
                        <dt class="text-on-surface-variant">Email</dt>
                        <dd class="text-on-surface font-medium">{{ $paciente->user?->email ?? '—' }}</dd>
                    </div>
                    <div class="flex justify-between">
                        <dt class="text-on-surface-variant">Telefone</dt>
                        <dd class="text-on-surface font-medium">{{ $paciente->user?->telefone ?? '—' }}</dd>
                    </div>
                    <div class="flex justify-between">
                        <dt class="text-on-surface-variant">Data de Início</dt>
                        <dd class="text-on-surface font-medium">{{ $paciente->data_inicio?->format('d/m/Y') ?? '—' }}</dd>
                    </div>
                </dl>
            </div>
            <div class="bg-white rounded-xl shadow-[0px_4px_12px_rgba(0,0,0,0.05)] border border-outline-variant/30 p-6">
                ...
                <dl class="space-y-3 text-body-sm">
                    <div>
                        ...
                        <dd class="text-on-surface">{{ $paciente->motivo_consulta ?? '—' }}</dd>
                    </div>
                    <div>
                        <dt class="text-on-surface-variant mb-1">Condições</dt>
                        <dd class="text-on-surface">{{ $paciente->condicoes ?? '—' }}</dd>
                    </div>
                    <div>
                        <dt class="text-on-surface-variant mb-1">Medicação</dt>
                        <dd class="text-on-surface">{{ $paciente->medicacao ?? '—' }}</dd>
                    </div>
                </dl>
            </div>
        </div>
                        ...
                        <tbody>
                            @foreach($paciente->consultas as $consulta)
                                ...
                                <tr class="border-b border-outline-variant/20">
                                    <td class="py-3 px-4 text-on-surface">{{ $consulta->data->format('d/m/Y') }}</td>
                                    <td class="py-3 px-4 text-on-surface">{{ \Carbon\Carbon::parse($consulta->hora)->format('H:i') }}</td>
                                    <td class="py-3 px-4 text-on-surface">{{ $consulta->tipo }}</td>
                                    <td class="py-3 px-4">
                                        <span class="font-label-md font-semibold px-2.5 py-1 rounded-full text-[11px] {{ $badgeClasses[$consulta->estado] ?? 'bg-surface-variant text-on-surface-variant' }}">
                                            {{ $consulta->estado }}
                                        </span>
                                    </td>
                                </tr>
        ...
        <div class="bg-white rounded-xl shadow-[0px_4px_12px_rgba(0,0,0,0.05)] border border-outline-variant/30 p-6">
            @if($paciente->subscricaoActiva?->plano)
                ...
                <div class="flex items-center justify-between mb-6">
                    <div>
                        <h3 class="font-title-lg text-title-lg text-on-surface">{{ $plano->nome }}</h3>
                        <p class="text-body-sm text-on-surface-variant mt-0.5">{{ $plano->descricao }}</p>
                    </div>
                ...
                <div class="mb-4">
                    <div class="flex justify-between text-body-sm mb-1">
                        ...
                        <span class="text-on-surface font-medium">{{ $sub->sessoes_usadas }}/{{ $plano->sessoes_total }} sessões</span>
                    </div>
                    <div class="w-full bg-surface-variant rounded-full h-2">
                        <div class="bg-primary h-2 rounded-full transition-all" style="width: {{ $progresso }}%"></div>
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-4 text-body-sm">
                    <div class="bg-surface-container-low rounded-lg p-3">
                        <p class="text-on-surface-variant">Sessões Restantes</p>
                        <p class="text-lg font-bold text-on-surface">{{ max(0, $restantes) }}</p>
                    </div>
                    <div class="bg-surface-container-low rounded-lg p-3">
                        <p class="text-on-surface-variant">Validade</p>
                        <p class="text-lg font-bold text-on-surface">{{ $sub->data_validade?->format('d/m/Y') ?? '—' }}</p>
                    </div>
                </div>
            @else
