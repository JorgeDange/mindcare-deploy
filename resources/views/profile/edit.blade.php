...
@section('title', 'Alterar Palavra-passe — MindCare')

@section('content')
<div class="max-w-[1000px] mx-auto">
    <!-- Breadcrumbs -->
    <nav class="flex items-center gap-2 mb-stack-lg text-on-surface-variant">
        <a href="{{ route('perfil') }}" class="font-body-sm text-body-sm hover:text-primary cursor-pointer transition-colors">Perfil</a>
        <span class="material-symbols-outlined text-sm">chevron_right</span>
        <a href="{{ route('perfil') }}#seguranca" class="font-body-sm text-body-sm hover:text-primary cursor-pointer transition-colors">Segurança</a>
        <span class="material-symbols-outlined text-sm">chevron_right</span>
    ...
    <div class="flex flex-col lg:flex-row gap-gutter">
        ...
        <section class="flex-[2] bg-white rounded-xl p-stack-lg shadow-[0px_4px_12px_rgba(0,0,0,0.05)] border border-outline-variant/10">
            ...
            <form method="post" action="{{ route('password.update') }}" class="space-y-stack-lg">
                @csrf
                @method('put')
                ...
                <div class="space-y-stack-sm">
                    ...
                    @if($errors->updatePassword->has('current_password'))
                        <p class="text-error text-xs font-bold mt-1 ml-1 flex items-center gap-1"><span class="material-symbols-outlined text-[14px]">warning</span> {{ $errors->updatePassword->first('current_password') }}</p>
                    @endif
                </div>
                ...
                <div class="space-y-stack-sm">
                    ...
                    @if($errors->updatePassword->has('password'))
                        <p class="text-error text-xs font-bold mt-1 ml-1 flex items-center gap-1"><span class="material-symbols-outlined text-[14px]">warning</span> {{ $errors->updatePassword->first('password') }}</p>
                    @endif
                ...
                <div class="space-y-stack-sm">
                    ...
                    @if($errors->updatePassword->has('password_confirmation'))
                        <p class="text-error text-xs font-bold mt-1 ml-1 flex items-center gap-1"><span class="material-symbols-outlined text-[14px]">warning</span> {{ $errors->updatePassword->first('password_confirmation') }}</p>
                    @endif
                </div>

                <div class="flex items-center justify-end gap-4 pt-4 border-t border-outline-variant/20">
                    <a href="{{ route('perfil') }}" class="px-6 h-12 flex items-center rounded-lg border border-primary text-primary font-body-md hover:bg-primary/5 transition-colors">Cancelar</a>
