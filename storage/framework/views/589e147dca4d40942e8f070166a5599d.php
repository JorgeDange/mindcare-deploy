<?php $__env->startSection('title', 'Planos de Saúde — MindCare'); ?>
...
<div class="max-w-container-max mx-auto space-y-stack-lg">
    ...
    <?php endif; ?>
    <?php if($temSubscricao && ($temPagamentoPendente ?? false)): ?>
    ...
    <section class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="md:col-span-2 bg-[#F2F8F8] p-8 rounded-xl border border-[#D1E6E6] relative overflow-hidden flex flex-col justify-between">
            ...
            <div>
                ...
                <div class="flex flex-col md:flex-row md:justify-between md:items-start mb-6 z-10 relative">
                    <div>
                        <h3 class="text-2xl font-bold text-[#065F5C] mb-1"><?php echo e($subscricaoActiva->plano->nome); ?></h3>
                        <p class="text-sm text-gray-600">Assinado em: <?php echo e($subscricaoActiva->data_inicio?->format('d \d\e F, Y')); ?></p>
                    </div>
                    <div class="mt-4 md:mt-0 text-left md:text-right">
                        <?php if($subscricaoActiva->plano->preco > 0): ?>
                            <div class="flex items-baseline md:justify-end gap-1">
                                <span class="text-4xl font-extrabold text-[#111827]">Kz <?php echo e(number_format($subscricaoActiva->plano->preco, 0, ',', '.')); ?></span>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-6 z-10 relative mt-auto">
                <?php $__currentLoopData = array_slice($beneficiosAtivo, 0, 4); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ben): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="flex items-center gap-3">
                    <span class="material-symbols-outlined text-[#065F5C]" style="font-variation-settings: 'FILL' 0;">check_circle</span>
                    <span class="text-sm font-medium text-gray-800"><?php echo e($ben); ?></span>
                </div>
        ...
        <div class="bg-[#6AA4F8] p-8 rounded-xl flex flex-col justify-between shadow-sm">
            <div>
                ...
                <p class="text-3xl font-extrabold text-[#14325E]"><?php echo e($subscricaoActiva->data_validade?->format('d \d\e M, Y')); ?></p>
            </div>
    ...
    <div id="troca-panel" class="<?php echo e($temSubscricao ? 'hidden' : ''); ?>">
        <?php
        ...
        <?php $__currentLoopData = $categorias; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $chave => $cat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <?php $planosCat = $planosVisiveis->where('publico', $chave)->values(); ?>
            <?php if($planosCat->isNotEmpty()): ?>
            <section class="space-y-6 mb-12">
                <div class="flex items-center gap-2">
                    <h3 class="text-2xl font-bold text-[#111827]">Outras Opções de Planos</h3>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 items-stretch">
                    <?php $__currentLoopData = $planosCat; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $plano): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <?php
                        ...
                        <?php if($isPlanoAtivo || $isDestacado): ?>
                        ...
                        <div class="bg-white p-8 rounded-xl border-[2.5px] border-[#065F5C] flex flex-col h-full relative">
                            ...
                            <h4 class="text-xl font-bold text-[#065F5C] mb-3"><?php echo e($plano->nome); ?></h4>
                            <p class="text-sm text-gray-600 mb-6 flex-grow"><?php echo e($plano->descricao); ?></p>
                            
                            <div class="mb-8">
                                <?php if($plano->preco > 0): ?>
                                    <div class="flex items-baseline gap-1">
                                        <span class="text-4xl font-extrabold text-[#111827]">Kz <?php echo e(number_format($plano->preco, 0, ',', '.')); ?></span>
                                        <span class="text-sm font-medium text-gray-600">/mês</span>
                                    </div>
                                <?php else: ?>
                                    <span class="text-3xl font-extrabold text-[#111827]">Sob consulta</span>
                            ...
                            <ul class="space-y-4 mb-8">
                                <?php $__currentLoopData = array_slice($beneficios, 0, 4); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $indexBen => $beneficio): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <li class="flex items-start gap-3 text-sm text-gray-800 <?php echo e($indexBen === 0 ? 'font-bold' : 'font-medium'); ?>">
                                    <span class="material-symbols-outlined text-gray-800 text-[20px]" style="font-variation-settings: 'wght' 600;">check</span>
                                    <span class="leading-tight"><?php echo e($beneficio); ?></span>
                                </li>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </ul>
                            ...
                            <div class="mt-auto">
                                ...
                                <?php else: ?>
                                <button onclick="abrirAdesaoOuTroca(<?php echo e($plano->id); ?>, <?php echo \Illuminate\Support\Js::from($plano->nome)->toHtml() ?>, <?php echo \Illuminate\Support\Js::from($plano->preco)->toHtml() ?>)"
                                        class="w-full bg-[#065F5C] text-white font-bold py-3.5 rounded-lg hover:bg-[#054E4A] transition-colors shadow-sm">
                                    <?php echo e($temSubscricao ? 'Migrar para ' : 'Aderir ao '); ?><?php echo e(explode(' ', $plano->nome)[0]); ?>
                        ...
                        <div class="bg-[#213535] p-8 rounded-xl flex flex-col h-full border border-transparent">
                            <h4 class="text-xl font-bold text-white mb-3"><?php echo e($plano->nome); ?></h4>
                            <p class="text-sm text-gray-300 mb-6 flex-grow"><?php echo e($plano->descricao); ?></p>
                            
                            <div class="mb-8">
                                <?php if($plano->preco > 0): ?>
                                    <div class="flex items-baseline gap-1">
                                        <span class="text-4xl font-extrabold text-white">Kz <?php echo e(number_format($plano->preco, 0, ',', '.')); ?></span>
                            ...
                            <ul class="space-y-4 mb-8">
                                <?php $__currentLoopData = array_slice($beneficios, 0, 4); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $beneficio): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            ...
                            <div class="mt-auto">
                                <button onclick="abrirAdesaoOuTroca(<?php echo e($plano->id); ?>, <?php echo \Illuminate\Support\Js::from($plano->nome)->toHtml() ?>, <?php echo \Illuminate\Support\Js::from($plano->preco)->toHtml() ?>)"
                                    ...
                                    <?php echo e($temSubscricao ? 'Upgrade para ' : 'Aderir ao '); ?><?php echo e(explode(' ', $plano->nome)[0]); ?>
                        ...
                        <div class="bg-white p-8 rounded-xl border border-gray-200 flex flex-col h-full">
                            <h4 class="text-xl font-bold text-[#111827] mb-3"><?php echo e($plano->nome); ?></h4>
                            <p class="text-sm text-gray-600 mb-6 flex-grow"><?php echo e($plano->descricao); ?></p>
    ...
    <section class="bg-[#DEF4F4] p-8 rounded-xl flex flex-col sm:flex-row items-center justify-between border border-[#CBEAEA] gap-6">
        ...
        <a href="<?php echo e(route('mensagens')); ?>" class="w-full sm:w-auto border-2 border-[#065F5C] text-[#065F5C] bg-white px-8 py-3.5 rounded-lg font-bold hover:bg-[#F2F8F8] transition-colors text-center whitespace-nowrap">
            Falar com Consultor
        </a>
    </section>
...
<div id="modal-adesao-troca" class="fixed inset-0 z-50 flex items-center justify-center bg-on-background/40 backdrop-blur-sm hidden p-4">
    ...
    <div class="bg-surface-container-lowest w-full max-w-[1000px] h-[90vh] max-h-[800px] rounded-[24px] shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in duration-300">
        ...
        <form action="<?php echo e($temSubscricao ? route('portal.plano.trocar') : route('portal.plano.aderir')); ?>" method="POST" enctype="multipart/form-data" class="flex flex-col h-full w-full">
            <?php echo csrf_field(); ?>
...
<script>
    const temSubscricao = <?php echo json_encode($temSubscricao, 15, 512) ?>;
...
<?php echo $__env->make('layouts.portal', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/vol16_2/infinityfree.com/if0_42474986/htdocs/resources/views/portal/plano.blade.php ENDPATH**/ ?>