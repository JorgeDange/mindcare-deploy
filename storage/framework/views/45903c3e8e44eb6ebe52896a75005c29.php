<?php $__env->startSection('title', 'Mensagens — MindCare'); ?>

<?php $__env->startSection('content'); ?>
<section class="flex h-full p-stack-md gap-stack-md overflow-hidden bg-surface-container-low">
    
    <!-- Contact List Column -->
    <div class="w-full md:w-1/3 bg-white rounded-2xl shadow-sm flex-col border border-outline-variant/30 overflow-hidden <?php echo e($conversaActiva ? 'hidden md:flex' : 'flex'); ?>">
        ...
        <div class="flex-1 overflow-y-auto chat-scroll divide-y divide-outline-variant/10">
            <?php $__empty_1 = true; $__currentLoopData = $owner->conversas; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $conversa): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <?php
                ...
                ?>
                <a href="<?php echo e(route('mensagens', ['conversa' => $conversa->id])); ?>" 
                   class="flex items-center gap-4 p-4 transition-colors cursor-pointer <?php echo e($conversaActiva && $conversaActiva->id === $conversa->id ? 'bg-surface-container-high border-l-4 border-primary' : 'hover:bg-surface-container-low'); ?>">
                    <div class="relative flex-shrink-0">
                        <?php if($prof && $prof->user && $prof->user->foto_perfil): ?>
                            <img src="<?php echo e(asset('storage/' . $prof->user->foto_perfil)); ?>" alt="Dr" class="h-12 w-12 rounded-full object-cover">
                        <?php else: ?>
                    ...
                    <div class="flex-1 overflow-hidden">
                        <div class="flex justify-between items-center">
                            <span class="font-bold text-on-surface truncate"><?php echo e($conversa->contacto); ?></span>
                            <span class="text-[10px] text-on-surface-variant flex-shrink-0 ml-1"><?php echo e($conversa->mensagens->last() ? $conversa->mensagens->last()->created_at->diffForHumans() : ''); ?></span>
                        </div>
                        <p class="text-body-sm text-on-surface-variant truncate mt-0.5">
                            <?php echo e($conversa->mensagens->last() ? Str::limit($conversa->mensagens->last()->texto, 35) : 'Sem mensagens'); ?>
    ...
    <div class="w-full md:flex-grow bg-white rounded-2xl shadow-sm flex-col overflow-hidden border border-outline-variant/30 relative <?php echo e($conversaActiva ? 'flex' : 'hidden md:flex'); ?>">
        <?php if($conversaActiva): ?>
            <?php $profActivo = $conversaActiva->profissional; ?>
            <!-- Chat Header -->
            <div class="p-4 border-b border-outline-variant/20 flex items-center justify-between bg-white/70 backdrop-blur-md sticky top-0 z-10">
                <div class="flex items-center gap-2 md:gap-4">
                    <a href="<?php echo e(route('mensagens')); ?>" class="md:hidden w-10 h-10 -ml-2 hover:bg-surface-container rounded-full text-on-surface-variant flex items-center justify-center">
                        <span class="material-symbols-outlined text-[20px]">arrow_back</span>
                    </a>
                    <?php if($profActivo && $profActivo->user && $profActivo->user->foto_perfil): ?>
                        <img src="<?php echo e(asset('storage/' . $profActivo->user->foto_perfil)); ?>" alt="Dr" class="h-10 w-10 rounded-full object-cover">
                    <?php else: ?>
                        <div class="h-10 w-10 rounded-full bg-secondary-container text-on-secondary-container flex items-center justify-center font-bold text-sm">
                            <?php echo e($conversaActiva->iniciais); ?>

                        </div>
                    <?php endif; ?>
                    <div>
                        <h3 class="font-bold text-on-surface text-sm leading-tight"><?php echo e($conversaActiva->contacto); ?></h3>
                ...
                <div class="flex gap-2">
                    <a href="<?php echo e(route('mensagens', ['conversa' => $conversaActiva->id])); ?>" class="w-10 h-10 hover:bg-surface-container rounded-full flex items-center justify-center text-on-surface-variant" title="Actualizar"><span class="material-symbols-outlined text-[20px]">refresh</span></a>
            ...
            <div class="flex-grow overflow-y-auto chat-scroll p-6 space-y-6 bg-[radial-gradient(#bdc9c8_1px,transparent_1px)] [background-size:24px_24px] bg-fixed chat-messages-area">
                ...
                <?php $__currentLoopData = $conversaActiva->mensagens; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $mensagem): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <?php
                        $msgDate = $mensagem->created_at->format('d \d\e F \d\e Y');
                    ?>
                    <?php if($currentDate !== $msgDate): ?>
                        <div class="flex justify-center my-4">
                            <span class="bg-surface-variant text-on-surface-variant px-3 py-1 rounded-full text-[10px] font-medium"><?php echo e($msgDate); ?></span>
                        </div>
                        <?php $currentDate = $msgDate; ?>
                    <?php endif; ?>

                    <?php if($mensagem->remetente_id !== Auth::id()): ?>
                        <!-- Message Received -->
                        <div class="flex items-end gap-3 max-w-[80%]" id="msg-<?php echo e($mensagem->id); ?>">
                            <?php if($profActivo && $profActivo->user && $profActivo->user->foto_perfil): ?>
                                <img src="<?php echo e(asset('storage/' . $profActivo->user->foto_perfil)); ?>" alt="Dr" class="h-8 w-8 rounded-full object-cover shrink-0 mb-1">
                            <?php else: ?>
                            ...
                            <div class="bg-white border border-outline-variant/30 p-4 rounded-2xl rounded-bl-none shadow-sm text-xs">
                                <p class="text-on-surface leading-normal"><?php echo nl2br(e($mensagem->texto)); ?></p>
                                <?php if($mensagem->anexo_path): ?>
...
<span class="w-8 h-8 bg-primary-fixed text-primary rounded-lg flex items-center justify-center flex-shrink-0"><span class="material-symbols-outlined text-[18px]">description</span></span>
                                        <div class="flex-grow min-w-0">
                                            <a href="<?php echo e(Storage::url($mensagem->anexo_path)); ?>" download class="font-bold text-on-surface truncate block text-[10px] hover:underline">
                                                <?php echo e(basename($mensagem->anexo_path)); ?>

                                            </a>
                                            <p class="text-[8px] text-on-surface-variant">Descarregar anexo</p>
                                        </div>
                                        <a href="<?php echo e(Storage::url($mensagem->anexo_path)); ?>" download class="material-symbols-outlined text-on-surface-variant text-[18px]">download</a>
                                    </div>
                                <?php endif; ?>
                                <span class="text-[9px] text-on-surface-variant block mt-2 text-right"><?php echo e($mensagem->created_at->format('H:i')); ?></span>
                            </div>
                        </div>
                    <?php else: ?>
                        <!-- Message Sent -->
                        <div class="flex items-end gap-3 max-w-[80%] ml-auto flex-row-reverse" id="msg-<?php echo e($mensagem->id); ?>">
                            <div class="bg-primary text-on-primary p-4 rounded-2xl rounded-br-none shadow-md text-xs">
                                <p class="leading-normal"><?php echo nl2br(e($mensagem->texto)); ?></p>
                                <?php if($mensagem->anexo_path): ?>
...
<span class="w-8 h-8 bg-white/10 text-white rounded-lg flex items-center justify-center flex-shrink-0"><span class="material-symbols-outlined text-[18px]">description</span></span>
                                        <div class="flex-grow min-w-0">
                                            <a href="<?php echo e(Storage::url($mensagem->anexo_path)); ?>" download class="font-bold text-white truncate block text-[10px] hover:underline">
                                                <?php echo e(basename($mensagem->anexo_path)); ?>

                                            </a>
                                            <p class="text-[8px] text-white/70">Descarregar anexo</p>
                                        </div>
                                        <a href="<?php echo e(Storage::url($mensagem->anexo_path)); ?>" download class="material-symbols-outlined text-white text-[18px]">download</a>
                                    </div>
                                <?php endif; ?>
                                <span class="text-[9px] text-primary-fixed/70 block mt-2 text-right"><?php echo e($mensagem->created_at->format('H:i')); ?></span>
                            </div>
                        </div>
                    <?php endif; ?>
            ...
            <!-- Message Input -->
            <form action="<?php echo e(route('portal.mensagens.store')); ?>" method="POST" enctype="multipart/form-data" class="p-4 bg-surface border-t border-outline-variant/20" style="margin: 0;">
                <?php echo csrf_field(); ?>
                <input type="hidden" name="conversa_id" value="<?php echo e($conversaActiva->id); ?>">
...
const conversaId = <?php echo e($conversaActiva ? $conversaActiva->id : 'null'); ?>;
const currentUserId = <?php echo e(Auth::id()); ?>;
const doctorAvatarUrl = <?php echo json_encode($conversaActiva && $profActivo && $profActivo->user && $profActivo->user->foto_perfil ? asset('storage/' . $profActivo->user->foto_perfil) : null, 15, 512) ?>;
const doctorInitials = <?php echo json_encode($conversaActiva ? $conversaActiva->iniciais : 'MC', 15, 512) ?>;
...
function startPolling() {
    pollInterval = setInterval(fetchNovasMensagens, 10000);
}
...
<?php echo $__env->make('layouts.portal', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/vol16_2/infinityfree.com/if0_42474986/htdocs/resources/views/portal/mensagens.blade.php ENDPATH**/ ?>